...
Pokud chcete změnit chování interních aplikací pro iOS a omezit jejich funkce, aniž byste měnili samotný kód, použijte nástroj Microsoft Intune App Wrapping pro iOS.

Tento nástroj je vlastně aplikace příkazového řádku systému Mac OS, která vytvoří obálku aplikace. Jakmile aplikaci zpracujete, můžete v Intune změnit její funkce pomocí [zásad správy mobilních aplikací](configure-and-deploy-mobile-application-management-policies-in-the-microsoft-intune-console.md), které jste nastavili.
...
Další informace o požadavcích a jejich nastavení najdete v tématu [Skype for Business Online: Povolení moderního ověřování tenanta](http://social.technet.microsoft.com/wiki/contents/articles/34339.skype-for-business-online-enable-your-tenant-for-modern-authentication.aspx).
...
1.  Soubory nástroje App Wrapping si můžete do počítače s Mac OS stáhnout z úložiště Microsoft Intune App Wrapping Tool for iOS [hostovaného na GitHubu](https://github.com/msintuneappsdk/intune-app-wrapping-tool-ios).
...
4.  Otevřete IntuneMAMPackager a uložte soubory do místní složky na počítači s Mac OS. Teď můžete nástroj App Wrapping spustit.

## <a name="run-the-app-wrapping-tool"></a>Spuštění nástroje pro zabalení aplikace
* Otevřete terminál a přejděte ke složce, kam jste uložili soubory nástroje App Wrapping. Spustitelný soubor má název IntuneMAMPackager a nachází se ve složce IntuneMAMPackager/Contents/MacOS. Spusťte následující příkaz:
...
./IntuneMAMPackager/Contents/MacOS/IntuneMAMPackager -i /<path of input app>/<app filename> -o /<path to output folder>/<app filename> -p /<path to provisioning profile> -c <SHA1 hash of the certificate> [-b [<output app build string>]] [-v] [-e] [-x /<array of extension provisioning profile paths>]
    ...
    **Příklad:** V následujícím příkladu se nástroj App Wrapping příkazem spouští na aplikaci MyApp.ipa. Je zadaný profil zřizování a hash SHA-1. Vytvoří se zpracovaná aplikace (MyApp_Wrapped.ipa), která se uloží do složky Desktop.
    ...
    ```
    V nástroji App Wrapping můžete používat následující vlastnosti příkazového řádku:
...
* Po zpracování se zobrazí zpráva „The application was successfully wrapped“ (Zabalení aplikace proběhlo úspěšně).
...
*   Zabalená aplikace se uloží do výstupní složky, kterou jste určili předtím. Aplikaci můžete nahrát do [wit_nextref](../includes/wit_nextref_md.md) a přidružit ji k zásadě správy mobilní aplikace.
    ...
    Teď můžete aplikaci nasadit do skupin [!INCLUDE[wit_nextref](../includes/wit_nextref_md.md)] a aplikace se bude na zařízení spouštět s vámi zadanými omezeními.
...
Aby bylo možné zaručit plnou funkčnost nástroje App Wrapping, je potřeba splnit určité požadavky.

|Požadavek|Podrobnosti|
|---------------|-----------|
|Profil pro zřizování|Zkontrolujte platnost zřizovacího profilu, než ho zahrnete. Při zpracování aplikace pro iOS nástroj App Wrapping nekontroluje, jestli vypršela platnost zřizovacího profilu. Když je zadaný profil zřizování s ukončenou platností, bude nástroj pro zabalení aplikace zahrnovat tento profil a vy nepoznáte, jestli existuje problém, dokud neselže instalace aplikace na zařízení s iOS.|
|Certifikát|Před zadáním certifikátu zkontrolujte jeho platnost. Nástroj při zpracování aplikací pro iOS nekontroluje, jestli nevypršela platnost certifikátu. Pokud je zadaný hash pro prošlý certifikát, nástroj zpracuje a podepíše aplikaci, ale nenainstaluje ji na zařízení.<br /><br />Zkontrolujte, jestli se certifikát dodaný k podpisu zabalené aplikace shoduje se zřizovacím profilem. Nástroj neověřuje, jestli pro certifikát poskytnutý k podepsání zabalené aplikace existuje shoda ve zřizovacím profilu.|
|Ověřování|Aby šifrování fungovalo, musí mít zařízení PIN. Když se uživatel zařízení, do kterého jste nasadili zabalenou aplikaci, dotkne stavového řádku, musí se znovu přihlásit ke službě [wit_nextref](../includes/wit_nextref_md.md). Podle výchozí zásady zabalené aplikace probíhá *ověřování při opakovaném spuštění*. V iOS se každé externí oznámení (třeba při telefonním hovoru) zpracuje tak, že se aplikace ukončí a potom znovu spustí.
...
1.  Povolení schopností v aplikaci:
    ...
    1.  V prostředí Xcode přejděte do cíle aplikace a klikněte na **Schopnosti**.
...
2.  Ve svém profilu pro zřizování povolte oprávnění:
    ...
    1.  Přihlaste se do Centra pro vývojáře Apple.
    ...
    4.  Dokončete průvodce zřizovacím profilem a stáhněte si soubor.

3.  Ujistěte se, že jste splnili všechny předpoklady, a zabalte aplikaci.

### <a name="troubleshooting-common-errors-with-entitlements"></a>Řešení potíží s běžnými chybami spojenými s oprávněními
...
### <a name="finding-the-existing-entitlements-of-a-signed-app"></a>Vyhledání stávajících oprávnění v podepsané aplikaci
...
-   Podpisový certifikát, zřizovací profil a obchodní aplikace, které zadáte, musí být na stejném počítači s Mac OS, na jakém spouštíte nástroj App Wrapping. Pokud soubory leží na cestě UNC, ověřte, že jsou pro počítač s Mac OS přístupné. Cesta musí být zabezpečená pomocí protokolu IPsec nebo podepsání SMB.
    ...
    Zabalená aplikace, kterou importujete do konzoly [wit_nextref](../includes/wit_nextref_md.md), musí být na stejném počítači, jaký používáte ke spuštění nástroje. Pokud soubor leží v cestě UNC, zajistěte, aby byl přístupný počítači se spuštěnou konzolou [wit_nextref](../includes/wit_nextref_md.md). Cesta musí být zabezpečená pomocí protokolu IPsec nebo podepsání SMB.
