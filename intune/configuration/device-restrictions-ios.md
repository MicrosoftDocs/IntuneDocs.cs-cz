...
- **Jednoduchá hesla**: vyberte **blok** pro vyžadování složitějších hesel. **Nenakonfigurováno** umožňuje jednoduchá hesla, například `0000` a `1234`.
...
- **Pro automatické vyplňování informací pro heslo nebo platební kartu vyžadovat ověření dotykového ID nebo ID obličeje**: Pokud je nastavené na **vyžadovat**, musí se uživatelé ověřit pomocí TouchID nebo FaceID před tím, než se můžou v Safari vyplní hesla nebo informace o platebních kartách. můžou. **Nenakonfigurováno** (výchozí) umožňuje uživatelům řídit tuto funkci v nastavení zařízení.
...
- **Povolení spravovaných aplikací pro psaní kontaktů na nespravované účty kontaktů**: Pokud je nastavené na **povoleno**, spravované aplikace, jako je například mobilní aplikace Outlook, můžou ukládat nebo synchronizovat kontaktní informace, včetně obchodních a firemních kontaktů, do integrovaných kontaktů iOS. aplikace. Pokud je nastavené na **Nenakonfigurováno** (výchozí), spravované aplikace nemůžou ukládat ani synchronizovat kontaktní informace na integrované aplikaci kontaktů pro iOS na zařízení.
...
- **Protokolování na straně serveru pro příkazy Siri**: Pokud je nastavení **zakázané**, protokolování Siri na straně serveru je vypnuté. Může také bránit protokolování požadavků uživatelů na servery Siri. **Nenakonfigurovaná** (výchozí) protokoluje příkazy Siri na straně serveru. Toto nastavení není závislé na nastavení Siri, které je blokováno nebo není nakonfigurováno.

  Tato funkce platí pro:  
  - iOS 12,2 a novější

  > [!NOTE]
  > Nastavení **příkazů protokolování na straně serveru pro příkazy Siri** se od společnosti Apple nepoužívá. V nadcházející verzi se toto nastavení odebere z konzoly Intune.
  >
  > V současné době toto nastavení nemá žádný vliv na zařízení, i když se toto nastavení zobrazuje v profilech Managemenet. Pokud chcete toto nastavení odstranit ze všech zásad, otevřete zásady, udělejte menší změnu a pak zásadu uložte. Zásada se aktualizuje a nastavení se odstraní ze zařízení.
...
- **Neoznačené e-mailové domény** > **Adresa URL e-mailové domény**: přidejte jednu nebo více adres URL do seznamu. Když koncoví uživatelé dostanou e-mail z jiné domény než z domén, které zadáte, e-mail se v aplikaci iOS mail označí jako nedůvěryhodný.
