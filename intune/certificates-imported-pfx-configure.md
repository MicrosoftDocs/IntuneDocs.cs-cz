...
- **Konektor certifikátu PFX pro Microsoft Intune**:  
  ...
  Další informace o všech síťových koncových bodech, ke kterým konektor přistupuje, najdete v tématu [požadavky na konfiguraci sítě Intune a šířku pásma](https://docs.microsoft.com/intune/network-bandwidth-use).
...
- **Visual Studio 2015 nebo vyšší** (volitelné): Pomocí sady Visual Studio sestavíte modul PowerShellu pomocníka s rutinami pro import certifikátů PFX do Microsoft Intune. Pokud chcete získat pomocné rutiny PowerShellu, přečtěte si téma [projekt PFXImport PowerShellu v GitHubu](https://github.com/microsoft/Intune-Resource-Access/tree/develop/src/PFXImportPowershell).
...
1. Na portálu [Intune](https://go.microsoft.com/fwlink/?linkid=2090973) vyberte **Konfigurace** > zařízení**certifikační konektory** > **Přidat** .
...
5. Okno zavřete.
6. Na portálu Intune se vraťte k části **Konfigurace** > zařízení**certifikační konektory**. Za chvíli se zobrazí zelená značka zaškrtnutí a **stav připojení** je **aktivní**. Server konektoru teď může komunikovat s Intune.
...
2. Pomocí souboru **PFXImportPS. sln**otevřete projekt pomocí sady Visual Studio. `.\Intune-Resource-Access-develop\src\PFXImportPowershell\`
...
6. Chcete-li vytvořit objekt **UserPFXCertificate** , spusťte příkaz`$userPFXObject = New-IntuneUserPfxCertificate -PathToPfxFile "<FullPathPFXToCert>" $SecureFilePassword "<UserUPN>" "<ProviderName>" "<KeyName>" "<IntendedPurpose>" "<PaddingScheme>"`
...
1. V portálu [Intune](https://go.microsoft.com/fwlink/?linkid=2090973) , ke kterým jste dostali **konfigurační** > **profily** > zařízení**vytvořit profil**.
