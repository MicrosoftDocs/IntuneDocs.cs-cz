...
description: "Naučte se registrovat zařízení s iOSem ve vlastnictví firmy pomocí Programu registrace zařízení (DEP) (nové uživatelské rozhraní).\""
keywords: 
author: ErikjeMS
ms.author: erikje
manager: dougeby
...
>
> Pokud ovšem vaše stránka **Registrace zařízení** vypadá jako obrázek níže, váš účet se ještě neaktualizoval na nové uživatelské rozhraní. Přejděte na [tuto stránku nápovědy](device-enrollment-program-enroll-ios.md).
>
...
1. V Intune na portálu Azure Portal zvolte **Registrace zařízení** > **Registrace Apple** > **Tokeny programu registrace** > **Přidat**.
...
2. Vyberte **Stáhnout veřejný klíč** a stáhněte si a místně uložte soubor šifrovacího klíče (.pem). Soubor .pem slouží k vyžádání certifikátu vztahu důvěryhodnosti z portálu Apple Device Enrollment Program.
  ![Snímek obrazovky s podoknem Token Programu registrace v pracovním prostoru Certifikáty Apple pro stažení veřejného klíče](./media/device-enrollment-program-enroll-ios/image02.png)
...
5. Pokud jste zvolili **Zaregistrovat s přidružením uživatele**, máte možnost povolit, aby se uživatelé ověřovali pomocí portálu společnosti místo v Průvodci nastavením společnosti Apple.
    ...
    > [!NOTE]
    > Vícefaktorové ověřování (MFA) nefunguje během registrace DEP, pokud máte vlastnosti profilu nastavené na **použití s přidružením uživatele** a nepoužíváte portál společnosti. Po registraci vícefaktorové ověřování na zařízeních funguje podle očekávání. Zařízení nemůžou vyzvat uživatele, kteří při prvním přihlášení potřebují změnit své heslo. Výzva k resetování hesla se během registrace nezobrazí ani uživatelům, kterým vypršela platnost hesla. Uživatelé musí heslo resetovat z jiného zařízení.
...
8. Zvolte, jestli chcete, aby zařízení, která používají tento profil, mohla **Synchronizovat s počítači**. Pokud zvolíte **Povolit Apple Configurator podle certifikátu**, musíte zvolit certifikát v části **Certifikáty Apple Configuratoru**.
