...
Dokončili jste kroky potřebné k začlenění komponenty do aplikace iOS založené na Xamarinu. Pokud k vytvoření projektu využíváte Xcode, můžete použít `Intune App SDK Settings.bundle`. To vám umožní při tvorbě projektu nastavení zásad Intune zapínat a vypínat kvůli testování a ladění. Pokud chcete využít výhod této sady, postupujte podle kroků v tématu [Intune App SDK pro iOS – Příručka vývojáře](app-sdk-ios.md) a přečtěte si část o [ladění v Xcodu](app-sdk-ios.md#status-result-and-debug-notifications).
