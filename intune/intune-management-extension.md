...
- Zařízení se systémem Windows 10 verze 1607 nebo novější. Pokud je zařízení zaregistrované pomocí [hromadného automatického zápisu](windows-bulk-enroll.md), musí na zařízeních běžet Windows 10 verze 1703 nebo novější. Rozšíření pro správu Intune není v režimu S Windows 10 podporované, protože režim S neumožňuje spouštění aplikací, které nejsou ve Storu. 
  
- Zařízení připojená k Azure Active Directory (AD), včetně:  
  
  - Hybridní služba Azure AD – připojeno: Zařízení připojená k Azure Active Directory (AD) a také připojená k místní službě Active Directory (AD). Pokyny najdete v tématu [Plánování implementace služby hybrid Azure Active Directory JOIN](https://docs.microsoft.com/azure/active-directory/devices/hybrid-azuread-join-plan) .
...
- Zařízení zaregistrovaná v Intune, včetně:
  ...
  - Ručně zaregistrovaná zařízení do Intune, což je v těchto případech:
    ...
    - [Automatický zápis do Intune](quickstart-setup-auto-enrollment.md) je povolený ve službě Azure AD. Koncový uživatel se k zařízení přihlásí pomocí místního uživatelského účtu, ručně připojí zařízení do služby Azure AD a pak se k zařízení přihlásí pomocí svého účtu Azure AD.
...
4. V **nastavení skriptu**zadejte následující vlastnosti a vyberte **Další**:
    ...
    - **Spusťte tento skript pomocí přihlašovacích údajů**přihlášeného: Vyberte **Ano** , pokud chcete skript spustit s přihlašovacími údaji uživatele v zařízení. Pokud chcete skript spustit v kontextu systému, vyberte **ne** (výchozí). Mnoho správců zvolí **Ano**. Pokud je nutné skript spustit v kontextu systému, klikněte na tlačítko **ne**.
    - **Vynutilo kontrolu signatury skriptů**: Vyberte **Ano** , pokud musí být skript podepsán důvěryhodným vydavatelem. Pokud není nutný žádný požadavek na podepsání skriptu, vyberte **ne** (výchozí). 
    - **Spustit skript v 64 hostitelském hostiteli PowerShellu**: Vyberte **Ano** , pokud chcete skript spustit v 64m hostiteli PowerShellu (PS) na 32bitové klientské architektuře s architekturou 64. Pokud vyberete **ne** (výchozí), spustí se skript v hostiteli powershellu 32.
...
5. Vyberte **značky oboru**. Značky oboru jsou volitelné. [Použijte řízení přístupu na základě role (RBAC) a značky oboru pro distribuované oddělení IT](scope-tags.md) s dalšími informacemi.
...
6. Vyberte **přiřazení** > **Vybrat skupiny, které chcete zahrnout**. Zobrazí se existující seznam skupin Azure AD.
    ...
    2. Vyberte **Další**.
        ...
        ![Přiřazení nebo nasazení skriptu PowerShellu do skupin zařízení v Microsoft Intune](./media/mgmt-extension-assignments.png)
...
Protokoly agenta v klientském počítači jsou obvykle v `\ProgramData\Microsoft\IntuneManagementExtension\Logs`systému. K zobrazení těchto souborů protokolu můžete použít [CMTrace. exe](https://docs.microsoft.com/sccm/core/support/tools) . 

![Snímek obrazovky nebo ukázkový protokol agenta CMTrace v Microsoft Intune](./media/apps-win32-app-10.png)  
...
### <a name="issue-intune-management-extension-doesnt-download"></a>Problém: Rozšíření správy Intune se nestahují.
...
Pokud chcete zjistit, jestli je zařízení automaticky zaregistrované, můžete:
  ...
  4. `MDMDiagReport` Otevřete ve webovém prohlížeči.
...
[Povolení automatické registrace Windows 10](windows-enroll.md#enable-windows-10-automatic-enrollment) zahrnuje postup konfigurace automatického zápisu v Intune.
