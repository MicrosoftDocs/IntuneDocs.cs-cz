...
2. Přejít na**profily** **Konfigurace** >  zařízení a vyberte **vytvořit profil**.

3. Zadejte podrobnosti o **názvu** a **popisu** profilu certifikátu PKCS.  

4. V rozevíracím seznamu **platforma** vyberte podporovanou platformu zařízení.

5. V rozevíracím seznamu **typ profilu** vyberte **certifikát PKCS**.
 
6. V podokně **certifikát PKCS** nakonfigurujte parametry s hodnotami z následující tabulky. Tyto hodnoty jsou potřeba k vydávání certifikátů PKCS z certifikační autority DigiCert prostřednictvím Intune Certificate Connectoru. 
...
7. Dokončete konfiguraci profilu tak, aby vyhovoval vašim obchodním potřebám, a pak výběrem **OK** profil uložte. 

8. Vyberte **přiřazení** a nakonfigurujte příslušnou skupinu, která bude tento profil přijímat. Aspoň jeden uživatel nebo zařízení musí být součástí přiřazené skupiny.
...
| --- | --- |
| Nepovedlo se přihlásit pomocí účtu správce tenanta Intune v uživatelském rozhraní konektoru NDES. | K tomu může dojít, když místní Certificate Connector není na portálu pro správu Intune povolený. Chcete-li tento problém vyřešit, použijte některý z následujících postupů: <br><br> Z uživatelského rozhraní Silverlight: <br> 1. Přihlaste se k [portálu pro správu Intune](https://admin.manage.microsoft.com). <br> 2. Vyberte **správce**. <br> 3. Vyberte**Certificate Connector** >  **pro správu mobilních zařízení**. <br> 4. Vyberte **Konfigurovat místní Certificate Connector**. <br> 5. Zaškrtněte políčko **Povolit Certificate Connector** . <br> 6. Vyberte **OK**. <br><br> Z uživatelského rozhraní Azure: <br> 1. Přihlaste se k webu [Azure Portal](https://portal.azure.com). <br> 2. Přejít na Microsoft Intune. <br> 3. Vyberte možnost **Konfigurace** > zařízení**certifikační autorita**. <br> 4. Vyberte **povolit**. <br><br> Po dokončení předchozích kroků z uživatelského rozhraní Silverlight nebo Azure Portal se zkuste přihlásit pomocí stejného účtu správce tenanta Intune v uživatelském rozhraní konektoru NDES. |
...
| NDES Connector – IssuePfx – obecná výjimka: <br> System. NullReferenceException: Odkaz na objekt není nastaven na instanci objektu. | Tato chyba je přechodná. Restartujte konektor služby Intune. <br><br> 1. Otevřete **Services. msc**. <br> 2. Vyberte službu **Intune Connector Service**. <br> 3. Klikněte pravým tlačítkem a vyberte možnost **Restartovat**. |
| Poskytovatel DigiCert – nepovedlo se získat zásady DigiCert. <br><br>Vypršel časový limit operace. | Intune Certificate Connector přijal při komunikaci s certifikační autoritou DigiCert chybu časového limitu operace. Pokud k této chybě dochází i nadále, zvyšte hodnotu časového limitu připojení a zkuste to znovu. <br><br> Chcete-li zvýšit časový limit připojení: <br> 1. Přejít na počítač konektoru NDES. <br>2. V programu Poznámkový blok otevřete soubor **%ProgramFiles%\Microsoft Intune\NDESConnectorSvc\NDESConnector.exe.config** . <br> 3. Zvyšte hodnotu časového limitu pro následující parametr: <br><br> `CloudCAConnTimeoutInMilliseconds` <br><br> 4. Restartujte službu Intune Certificate Connector. <br><br> Pokud se problém nevyřeší, obraťte se na zákaznickou podporu DigiCert. |
