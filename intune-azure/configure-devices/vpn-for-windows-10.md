...
**Vlastní XML**/**Vlastní data XML protokolu EAP** – zadejte vlastní příkazy XML pro konfiguraci připojení VPN.
...
**Rozdělit tunel** – tuto možnost můžete **Povolit** nebo **Zakázat**, aby se mohla zařízení rozhodnout, které připojení se má v závislosti na typech přenosů používat. Například uživatel v hotelu bude pro přístup k pracovním souborům používat připojení VPN, ale pro běžné procházení webu bude používat standardní síť hotelu.
- **Rozdělit tunelové trasy pro toto připojení k síti VPN** – přidejte volitelné trasy pro poskytovatele připojení VPN z kategorie třetích stran. Pro každou trasu zadejte předponu cíle a velikost předpony.
...
**Pravidla síťových přenosů pro toto připojení k síti VPN** – vyberte, které protokoly a které rozsahy místních a vzdálených portů a adres budou povolené pro připojení VPN. Když nevytvoříte pravidlo pro provoz sítě, budou povolené všechny protokoly, porty a rozsahy adres. Po vytvoření pravidla se pro připojení VPN použijí jenom protokoly, porty a rozsahy adres, které určíte v tomto pravidle.
...
**Názvy a servery DNS pro toto připojení k síti VPN** – vyberte, které servery DNS bude toto připojení VPN používat po vytvoření připojení.
